"""
Script to add trial-specific and word-specific events to the data, instead of only the detected
trigger signals showing the start of each stimuli within the trial.
"""
...
if __name__ == "__main__":
    ...
    raw = mne.io.read_raw_fif(bids_path.copy().update(extension=".fif"), preload=True)
    ...
    if args.miniblock_events:
        for k, v in lut_triggers.items():
            parts = v.split("/")
            if parts[0] == "LOCALIZER":
                lut_triggers[k] = "".join(parts[1:])
                parts = parts[1:]
                v = "".join(parts[1:])
            if parts[0] in ("NONWORD", "SENTENCE"):
    ...
    for row in evdf.itertuples():
        ...
        if args.miniblock_events and row.label2.split("/")[0] in ("SENTENCE", "NONWORD"):
    ...
    bidsevs = pd.DataFrame.from_records(records).sort_values("onset")
